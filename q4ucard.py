...
from smartcard.CardMonitoring import CardMonitor, CardObserver
from smartcard.Exceptions import CardConnectionException, ListReadersException
...
class PrintObserver(CardObserver):
    ...
    def readConfig(self):
      with open('./config.json') as json_file:
        ...
        self.kioskId = data['kioskId']
        self.urlAPI = data['urlAPI']
        self.token = data['token']
        print(data)

    def initCardReader(self):
      self.apdu = SELECT+THAI_ID_CARD
      ...
      # stat = self.cardservice.connection.connect()
      # print('CONNECTED TO '+self.cardservice.connection.getReader());

    def getInfo(self,card):
      print(card)
      self.apdu = SELECT+THAI_ID_CARD
      # cardtype = AnyCardType()
      ...
      try:
        for d in DATA:
            response, sw1, sw2 = card.connection.transmit( d )
            if sw1 == 0x61:
                GET_RESPONSE = [0X00, 0XC0, 0x00, 0x00 ]
                self.apdu = GET_RESPONSE + [sw2]
                response, sw1, sw2 = card.connection.transmit( self.apdu )
                result = ''
                for i in response:
      ...
      return resultdict#json.dumps(resultdict)

    def onInserted(self,card):
      cardInfo = self.getInfo(card)
      if cardInfo['status'] == 'inactive':
        return
      cid = cardInfo['idnumber']
      thName = cardInfo['thainame']
      dob = "{}/{}/{}".format(cardInfo['dob'][6:8],cardInfo['dob'][4:6],cardInfo['dob'][0:4])
      prefix = thName.split('#')[0]
      fname = thName.split('#')[1]
      lname = thName.split('##')[1].strip()
      data = "token={}&kioskId={}&cid={}&title={}&fname={}&lname={}&birthDate={}".format(self.token,self.kioskId,cid,prefix,fname,lname,dob)
      print(data)
      headers = {
        'Content-Type': 'application/x-www-form-urlencoded',
        'cache-control': 'no-cache'
      }
      print("{}/kiosk/profile".format(self.urlAPI))
      r = requests.post("{}/kiosk/profile".format(self.urlAPI), data = data.encode('utf-8'), headers = headers)
      print(r)
    def onRemoved(self):
        data = "token={}&kioskId={}".format(self.token,self.kioskId)
        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        r = requests.delete("{}/kiosk/profile".format(self.urlAPI), data = data, headers = headers)
        print(r)
    def update(self, observable, actions):
        (addedcards, removedcards) = actions
        for card in addedcards:
            print("+Inserted: ", toHexString(card.atr))
            self.onInserted(card)
